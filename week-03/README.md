...
> Agent 开发实战 15 周 · 第 3 周 · 实际投入约 6 小时
>
...
这就是 W03 的主题：**Tool Use（工具调用）**。它把 LLM 从一个"文本生成器"变成了一个"能操作你系统的决策者"。所有框架（LangChain、LangGraph、OpenAI Agents SDK）做的第一件事，就是把本周这个循环工程化——所以本周必须亲手写出来，不能靠框架糊弄过去。
